...
int handleEvents(struct pollfd* connections, const nfds_t* connections_qty){
    ...
    for(int i = 0; i < *connections_qty; ++i){
        ...
        if(connections[i].revents & (POLLIN | POLLOUT)){
            ...
            if(task1){
                setOnExecution(&tasks, connections[i].fd);
            }
            else{
                taskAppend(&tasks, createTask(REQUEST, connections[i].fd));
            }
            connections[i].fd *= -1;
            compress_array = 1;
            --handled;
...
int main(int argc, char* argv[]) {
    ...
    if(listen_port < 1024 || thread_pool_size < 1){
        printf("bad input");
        exit(EXIT_FAILURE);
    }
    ...
    for(int i = 0; i < thread_pool_size; ++i){
        if(pthread_create(&thread_pool[i], NULL, workThread, &threadArguments) != 0){
            free(connections);
            free(thread_pool);
